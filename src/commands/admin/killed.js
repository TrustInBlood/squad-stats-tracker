...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (kills.length === 0) {
        await transaction.rollback();
        return interaction.editReply(`No deaths found for ${player.last_known_name} in the last ${hours} hours.`);
      }
      ...
      logger.info(`Killed command executed for player_id: ${player.id}, hours: ${hours}`);

    } catch (error) {
      await transaction.rollback();
      logger.error(`Error in killed command: ${error.message}`);
